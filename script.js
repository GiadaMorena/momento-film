...
function initializeApp(user) {
    ...
    // UNICA FONTE DI VERITÀ: onSnapshot ascolta le modifiche da Firebase
    docRef.onSnapshot((doc) => {
        ...
        // Ogni volta che i dati cambiano, l'intera UI viene ridisegnata da zero
        renderUI(appState);
    }, (error) => {
        console.error("Errore nell'ascolto dei dati: ", error);
    });
    
    // Funzione centrale per ridisegnare tutta l'applicazione
    ...
    // Funzione per aggiornare i dati su Firestore
    async function updateMovieData(movieId, movieData) {
        const updatePayload = {};
        for (const key in movieData) {
            updatePayload[`movies.${movieId}.${key}`] = movieData[key];
        }
        // Gestisce la rimozione del vecchio campo `rating`
        updatePayload[`movies.${movieId}.rating`] = firebase.firestore.FieldValue.delete();
    ...
    function openModal(movieId) {
        ...
        saveBtn.addEventListener('click', () => {
            const movieUpdate = {
                ratings: {
                    m: parseFloat(ratingM_input.value) || 0,
                    g: parseFloat(ratingG_input.value) || 0,
                },
    ...
    function renderMovies(state) {
        ...
        filteredList.sort((a, b) => {
            ...
            const ratingsA = state.movies[a.id]?.ratings || { m: 0, g: 0 };
            const votesA = Object.values(ratingsA).filter(r => r > 0);
            const averageA = votesA.length > 0 ? votesA.reduce((sum, val) => sum + parseFloat(val), 0) / votesA.length : 0;
            const ratingsB = state.movies[b.id]?.ratings || { m: 0, g: 0 };
            const votesB = Object.values(ratingsB).filter(r => r > 0);
            ...
            switch (sortOrder) {
                ...
                case 'rating-desc': const ratingDiffDesc = averageB - averageA; if (ratingDiffDesc !== 0) return ratingDiffDesc; return yearB - yearA;
                case 'rating-asc': const ratingDiffAsc = averageA - averageB; if (ratingDiffAsc !== 0) return ratingDiffAsc; return yearA - yearB;
    ...
    // --- Funzioni di Utilità (la maggior parte non cambia) ---
    ...
    async function exportSeenMovies() { /* Implementazione invariata */ }
    ...
    themeToggleBtn.addEventListener('click', () => updateState({ theme: appState.theme === 'dark' ? 'light' : 'dark' }));
    ...
    sortOrderSelect.addEventListener('change', () => { const value = sortOrderSelect.value; if (value.includes('year')) { sortIcon.className = 'fas fa-calendar-alt'; } else if (value.includes('title')) { sortIcon.className = 'fas fa-sort-alpha-down'; } else if (value.includes('rating')) { sortIcon.className = 'fas fa-star'; } else { sortIcon.className = 'fas fa-clock'; } renderMovies(appState); });
