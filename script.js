document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMovies() {
        ...
        const groupedMovies = filmList.reduce((acc, movie) => { acc[movie.category] = acc[movie.category] || []; acc[movie.category].push(movie); return acc; }, {});
        ...
        for (const category in groupedMovies) {
            ...
            groupedMovies[category].forEach(movie => {
                ...
                // RIATTIVIAMO L'INVISIBILITÀ INIZIALE PER L'ANIMAZIONE
        ...
        // RIATTIVIAMO L'ANIMAZIONE DI FRAMER MOTION
    ...
    // NUOVA FUNZIONE PER ANIMARE I CONTATORI
    ...
    function updateStatsDashboard(isInitialLoad = false) {
        ...
        if (isInitialLoad) {
            seenCounterEl.textContent = seenCount;
            favCounterEl.textContent = favoriteCount;
            completionEl.textContent = `${completionPercentage.toFixed(1)}%`;
        } else {
            const oldSeen = parseInt(seenCounterEl.textContent) || 0;
            const oldFav = parseInt(favCounterEl.textContent) || 0;
    ...
    function exportSeenMovies() { /* ... identica a prima, ma aggiornata per 'seen' unificato ... */ }
    ...
    loadState();
    renderMovies();
    updateStatsDashboard(true); // Caricamento iniziale senza animazione
});