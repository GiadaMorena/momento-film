document.addEventListener('DOMContentLoaded', () => {
    ...
    // Un unico oggetto per tenere traccia di tutto.
    let appState = {
        theme: 'dark',
        movies: {} // Esempio: { 1: { seenMaria: true, seenGiada: false, favorite: true, note: "..." }, ... }
    };
    ...
    function renderMovies() {
        ...
        for (const category in groupedMovies) {
            ...
            groupedMovies[category].forEach(movie => {
                ...
                // Animazione con Framer Motion (opzionale ma carino)
        ...
        // Applica le animazioni dopo aver aggiunto gli elementi al DOM
        const motion = window.motion; // da Framer Motion CDN
        if (motion) {
            motion.stagger(0.05, [
    ...
    function updateProgress() {
        const totalMovies = filmList.length;
        let mariaSeenCount = 0;
        let giadaSeenCount = 0;

        Object.values(appState.movies).forEach(state => {
            if (state.seenMaria) mariaSeenCount++;
            if (state.seenGiada) giadaSeenCount++;
        });
    ...
    // Gestisce il click su checkbox, preferiti e dettagli
    function handleContainerClick(e) {
        ...
        // Inizializza lo stato del film se non esiste
        ...
        if (target.type === 'checkbox') {
            ...
            // Salva la data della prima volta che viene spuntato
        ...
        // Gestione Dettagli (apre modale)
        if (target.classList.contains('details-btn')) {
            openModal(movieId);
        }
    }

    // Apre e popola la modale
    function openModal(movieId) {
        ...
        document.getElementById('save-modal-btn').addEventListener('click', () => {
            movieState.note = document.getElementById('movie-note').value;
            movieState.rating = document.getElementById('movie-rating').value;
            saveState();
            closeModal();
        });
    }
    ...
    // Esporta la lista dei film visti
