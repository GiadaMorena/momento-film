document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 2. STATO DELL'APPLICAZIONE (invariato) ---
    let appState = {
        theme: 'dark',
        movies: {} 
    };

    // --- 3. SELETTORI DEL DOM (invariati) ---
    ...
    // --- 4. FUNZIONI PRINCIPALI (aggiornate) ---
                ...
                card.innerHTML = `
                    ...
                    <div class="checkbox-container">
                        <label class="checkbox-label">
                            <input type="checkbox" data-id="${movie.id}" ${movieState.seen ? 'checked' : ''}> Visto
                        </label>
                    </div>
    ...
    function updateStatsDashboard() {
        ...
        document.getElementById('stats-seen-count').textContent = seenCount;
        document.getElementById('stats-favorite-count').textContent = favoriteCount;
        document.getElementById('stats-completion-percentage').textContent = `${completionPercentage.toFixed(1)}%`;
    }
    ...
    // Le altre funzioni (openModal, closeModal, toggleTheme, ecc.) rimangono le stesse
    ...
    loadState();
    renderMovies();
    updateStatsDashboard(); // Chiamiamo la nuova funzione
});